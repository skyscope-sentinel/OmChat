    ...
    def __call__(
        ...
        if images is not None:
            ...
            for img in image_inputs:
                ...
                n = len(img)
                #image_tensor= torch.stack(img, dim=0)
                image_tensor = img
                ...
                new_texts.extend(context_tokens[1])
            text_inputs = torch.tensor(new_texts)
        else:
            image_inputs = {}
            context_tokens = make_context(
                    tokenizer,
        ...
        if images is not None:
            ...
            if len(img) == 1:
                ...
                for i, n in enumerate(num_patches.tolist()):
                    ...
                    if i+1 < len(texts):
                        final += texts[i+1]
                #inp, context_tokens = make_context(self.tokenizer, final, None, "You are a helpful assistant. Focus on accuracy and reliability in your response.")
