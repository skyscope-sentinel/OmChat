...
from transformers.utils import logging
from transformers import AutoConfig as InternVisionConfig
...
class OmChatForConditionalGeneration(OmChatPreTrainedModel):
    ...
    def __ffeature_select(self, image_forward_outs):
    ...
    def feature_select(self, image_forward_outs, layers=[12,16,22,23]):
        ...
        return None, image_features_multi    
    ...
    def vision_tower_forward2(self, images):        
        if type(images) is list:
            image_features = []
            for image in images:
                image_forward_out = self.vision_tower(image.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_hidden_states=True)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (vision_tower is None and video_tower is None) or images is None or input_ids.shape[1] == 1:
            ...
            #if type(past_key_values) is torch.tensor  and (vision_tower is not None or video_tower is not None) and images is not None and input_ids.shape[1] == 1:
    ...
    def prepare_inputs_for_generation4(
    ...
    def prepare_inputs_for_generation(
        ...
        # Only add token_type_ids if it's not None
        if token_type_ids is not None:
